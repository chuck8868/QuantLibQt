...
namespace {
    ...
    struct CommonVars {
        ...
        std::vector<boost::shared_ptr<CmsCouponPricer> > numericalPricers;
        std::vector<boost::shared_ptr<CmsCouponPricer> > analyticPricers;
        ...
        CommonVars() {
            ...
            SabrVolCube2 = Handle<SwaptionVolatilityStructure>(
                boost::shared_ptr<SwaptionVolCube2>(new
            ...
            SabrVolCube1 = Handle<SwaptionVolatilityStructure>(
                boost::shared_ptr<SwaptionVolCube1>(new
            ...
            for (Size j=0; j<yieldCurveModels.size(); ++j) {
                numericalPricers.push_back(boost::shared_ptr<CmsCouponPricer>(new
                    NumericHaganPricer(atmVol, yieldCurveModels[j],
                                       zeroMeanRev)));
                analyticPricers.push_back(boost::shared_ptr<CmsCouponPricer>(new
...
int main(int, char* []) {
    ...
    try {
        ...
        // Pricing
        for (Size j=0; j<yieldCurveModels.size(); ++j) {
            std::cout << yieldCurveModels[j] << std::endl;
            numericalPricers[j]->setSwaptionVolatility(atmVol);
            analyticPricers[j]->setSwaptionVolatility(atmVol);
            for (Size sl=0; sl<n; ++sl) {
                setCouponPricer(cms[sl]->leg(0), numericalPricers[j]);
                Real priceNum = cms[sl]->NPV();
                setCouponPricer(cms[sl]->leg(0), analyticPricers[j]);
                Real priceAn = cms[sl]->NPV();
                ...
                //if (difference > tol)
                    BOOST_FAIL("\nLength in Years:  " << swapLengths[sl] <<
                               ...
                               //"\ngearing:          " << io::rate(gearing) <<
                               "\nswap index:       " << swapIndex->name() <<
                               "\nibor index:       " << iborIndex->name() <<
                               "\nspread:           " << io::rate(spread) <<
                               //"\ncap:              " << io::rate(infiniteCap) <<
                               "\nYieldCurve Model: " << yieldCurveModels[j] <<
                               "\nNumerical Pricer: " << io::rate(priceNum) <<
                               "\nAnalytic Pricer:  " << io::rate(priceAn) <<
                               "\ndifference:       " << io::rate(difference) <<
                               "\ntolerance:        " << io::rate(tol));
